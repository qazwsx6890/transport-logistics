    ...
    def call(self, func, *args, **kwargs):
        if self.state == 'OPEN':
            if (datetime.now() - self.last_failure_time).seconds > self.timeout_seconds:
                ...
                raise Exception(f"Circuit Breaker OPEN. Повторите через {self.timeout_seconds} сек")
...
<body>
    <div class="container">
        <div class="main-card">
            ...
            <div class="content">
                    ...
                    <div class="col-md-6">
                        <div class="card-custom">
                            ...
                            <div class="d-flex gap-3 mt-3">
                                <button class="btn btn-success-custom btn-custom flex-fill" onclick="createRequest(false)">
                                    <i class="fas fa-check me-2"></i>Успешный сценарий
                                </button>
                                <button class="btn btn-danger-custom btn-custom flex-fill" onclick="createRequest(true)">
                                    <i class="fas fa-exclamation-triangle me-2"></i>Сценарий с ошибкой
                                </button>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Правая колонка - Circuit Breaker -->
                    <div class="col-md-6">
                        <div class="circuit-card" id="circuitCard">
                            ...
                            <button class="btn btn-light mt-3" onclick="resetCircuit()" style="border-radius: 10px;">
                                <i class="fas fa-sync me-2"></i>Сбросить Circuit Breaker
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <!-- Логи -->
                <h5><i class="fas fa-terminal me-2" style="color: #667eea;"></i>Логи интеграции (JSON формат)</h5>
                <div class="log-container" id="logArea">
                    <div class="log-info">[INFO] Система готова к работе</div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-custom">
                        <thead>
                            <tr><th>ID</th><th>Время</th><th>Вес</th><th>Маршрут</th><th>Статус</th><th>Ошибка</th></tr>
                        </thead>
                        <tbody id="historyTable">
                            <tr><td colspan="6" class="text-center">Нет заявок</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    <script>
        ...
        async function createRequest(forceError) {
            addLog(`Создание заявки (тест ошибки: ${forceError})`, 'info');
            try {
        ...
        async function loadHistory() {
            ...
            if (requests.length === 0) {
                ...
                tbody.innerHTML = requests.map(r => `
                    <tr style="background: ${r.status === 'error' ? '#fff5f5' : '#f0fff4'}">
        ...
        async function loadCircuitStatus() {
            ...
            if (data.state === 'CLOSED') {
                card.classList.add('circuit-closed');
                statusDiv.innerHTML = '<i class="fas fa-check-circle me-2"></i> СОСТОЯНИЕ: CLOSED (работает)';
            } else if (data.state === 'OPEN') {
                card.classList.add('circuit-open');
                statusDiv.innerHTML = '<i class="fas fa-ban me-2"></i> СОСТОЯНИЕ: OPEN (заблокирован)';
            } else {
                card.classList.add('circuit-half');
                statusDiv.innerHTML = '<i class="fas fa-chart-line me-2"></i> СОСТОЯНИЕ: HALF-OPEN (тестовый режим)';
            }
...
def create_request():
    ...
    logger.info(f"Создание заявки {request_id}", extra={'request_id': request_id})
    ...
    try:
        ...
        logger.info(f"Заявка {request_id} успешно создана", extra={'request_id': request_id})
        ...
        error_msg = str(e)
        logger.error(f"Ошибка заявки {request_id}: {error_msg}", extra={'request_id': request_id})
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("Логи: logs/app.log (JSON формат)")
